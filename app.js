...
if (Array.isArray(window.CLIENTES_INICIAIS) && window.CLIENTES_INICIAIS.length) {
  const iniciais = JSON.parse(JSON.stringify(window.CLIENTES_INICIAIS));
  let alterouBase = false;

  iniciais.forEach(clienteInicial => {
    const jaExiste = clientes.some(c =>
      c.id === clienteInicial.id ||
      (c.cpf && clienteInicial.cpf && c.cpf === clienteInicial.cpf)
    );

    if (!jaExiste) {
      clientes.push(clienteInicial);
      alterouBase = true;
    }
  });

  if (alterouBase || !localStorage.getItem(STORAGE_KEY)) {
...
function abrirDetalhes(id) {
  ...
  $("#detalheConteudo").innerHTML = `
    ...
    <div class="detail-toolbar">
      <button class="btn btn-primary" onclick="gerarPDFCliente('${c.id}')">Gerar PDF</button>
    </div>
...
function gerarPDFCliente(id) {
  ...
  const pendentes = c.parcelas.filter(p => !p.pago && statusParcela(p) === "pendente").length;
  const atrasadas = c.parcelas.filter(p => !p.pago && statusParcela(p) === "atrasado").length;
  const percentual = r.total > 0 ? (r.recebido / r.total) * 100 : 0;

  const linhas = c.parcelas.map(p => {
    const s = statusParcela(p);
    return `
      <tr>
        <td>${p.numero}ª</td>
        <td>${dataBR(p.vencimento)}</td>
        <td>${moeda(p.valor)}</td>
        <td><span class="status ${s}">${labelStatus(s)}</span></td>
        <td>${p.dataPagamento ? dataBR(p.dataPagamento) : "-"}</td>
      </tr>
    `;
  }).join("");
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .report-date { text-align:right; color:#64748b; line-height:1.5; }
  .client-box {
  ...
  .client-name { font-size:16px; font-weight:800; margin-bottom:8px; color:#111827; }
  .client-data { display:grid; grid-template-columns:1fr 1fr; gap:7px 18px; }
  .label { color:#64748b; font-size:9px; text-transform:uppercase; font-weight:700; }
  .value { margin-top:2px; font-weight:700; }
  .stats { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; margin-bottom:14px; }
  .stat {
    border:1px solid #dbe3ef; border-radius:9px; padding:10px;
    break-inside:avoid;
  }
  .stat span { display:block; color:#64748b; font-size:9px; margin-bottom:4px; }
  .stat strong { font-size:13px; color:#111827; }
  .progress-wrap { margin: 0 0 16px; }
  .progress-info { display:flex; justify-content:space-between; margin-bottom:5px; color:#475569; }
  .progress { height:7px; border-radius:99px; background:#e2e8f0; overflow:hidden; }
  .progress div { height:100%; background:#2563eb; width:${Math.min(100, percentual).toFixed(2)}%; }
  h2 { font-size:13px; margin:17px 0 8px; color:#111827; }
  .obs { padding:10px; border-left:3px solid #2563eb; background:#f8fafc; margin-bottom:14px; }
  table { width:100%; border-collapse:collapse; }
  thead { display:table-header-group; }
  tr { break-inside:avoid; }
  th {
    background:#172033; color:white; text-align:left;
    padding:7px 6px; font-size:9px;
  }
  td { padding:7px 6px; border-bottom:1px solid #e2e8f0; font-size:9.5px; }
  .status { font-weight:800; font-size:8px; text-transform:uppercase; }
  .status.pago { color:#15803d; }
  .status.pendente { color:#b45309; }
  .status.atrasado { color:#dc2626; }
  .summary-line {
    display:flex; gap:16px; flex-wrap:wrap;
    margin:9px 0 13px; color:#475569;
  }
  .footer {
    margin-top:18px; padding-top:9px; border-top:1px solid #dbe3ef;
...
<body>
  <button class="no-print" onclick="window.print()">Salvar / Imprimir PDF</button>
  ...
  <div class="header">
    <div class="brand">
      ...
      <div>
        ...
        <div class="subtitle">Marcelo Barreto - Controle Financeiro</div>
      </div>
    </div>
  ...
  <div class="client-box">
    ...
    <div class="client-data">
      <div><div class="label">CPF</div><div class="value">${esc(c.cpf || "-")}</div></div>
      <div><div class="label">Contato</div><div class="value">${esc(c.contato || "-")}</div></div>
      <div><div class="label">Quantidade de parcelas</div><div class="value">${c.parcelas.length}</div></div>
      <div><div class="label">Parcelas pagas</div><div class="value">${pagas} de ${c.parcelas.length}</div></div>
    </div>
  </div>

  <div class="stats">
    <div class="stat"><span>VALOR TOTAL</span><strong>${moeda(r.total)}</strong></div>
    <div class="stat"><span>TOTAL RECEBIDO</span><strong>${moeda(r.recebido)}</strong></div>
    <div class="stat"><span>SALDO EM ABERTO</span><strong>${moeda(r.aberto)}</strong></div>
    <div class="stat"><span>VALOR EM ATRASO</span><strong>${moeda(r.atrasado)}</strong></div>
  </div>
  ...
  <div class="summary-line">
    <span><strong>${pagas}</strong> paga(s)</span>
    <span><strong>${pendentes}</strong> pendente(s)</span>
    <span><strong>${atrasadas}</strong> atrasada(s)</span>
  </div>

  ${c.observacoes ? `<div class="obs"><strong>Observações:</strong> ${esc(c.observacoes)}</div>` : ""}
  ...
  <div class="footer">
    Documento gerado pelo sistema Marcelo Barreto - Controle Financeiro.
  </div>
</body>
</html>`;
...
function baixarArquivo(nome, conteudo, tipo = "application/octet-stream") {
...
function exportarBackup() {
  const backup = { sistema: "Controle Financeiro - Marcelo Barreto", versao: 1, exportadoEm: new Date().toISOString(), clientes };
  baixarArquivo(`backup-controle-financeiro-${hojeISO()}.json`, JSON.stringify(backup, null, 2), "application/json;charset=utf-8");
}

function validarClientesImportados(lista) {
  return Array.isArray(lista) && lista.every(c => c && typeof c.nome === "string" && Array.isArray(c.parcelas));
}

async function importarBackupArquivo(arquivo) {
  try {
    const dados = JSON.parse(await arquivo.text());
    const lista = Array.isArray(dados) ? dados : dados.clientes;
    if (!validarClientesImportados(lista)) return alert("Backup inválido.");
    if (!confirm(`O backup possui ${lista.length} cliente(s). Os dados atuais serão substituídos. Continuar?`)) return;
    clientes = JSON.parse(JSON.stringify(lista));
    localStorage.setItem(STORAGE_KEY, JSON.stringify(clientes));
    renderTudo();
...
function gerarDadosIniciais() {
  const conteudo = `// Gerado pelo Controle Financeiro - Marcelo Barreto\nwindow.CLIENTES_INICIAIS = ${JSON.stringify(clientes, null, 2)};\n`;
  baixarArquivo("dados-iniciais.js", conteudo, "text/javascript;charset=utf-8");
}
...
$("#arquivoBackup").addEventListener("change", async (e) => { const arquivo = e.target.files?.[0]; if (arquivo) await importarBackupArquivo(arquivo); e.target.value = ""; });
$("#btnGerarDadosIniciais").addEventListener("click", gerarDadosIniciais);
